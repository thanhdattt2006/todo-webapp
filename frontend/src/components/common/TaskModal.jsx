...
export default function TaskModal({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          'prose prose-sm dark:prose-invert focus:outline-none min-h-[150px] max-w-none text-content-main-light dark:text-content-main-dark',
      },
    },
  });
          ...
          <div className='flex min-h-full items-center justify-center p-4 text-center'>
            ...
            >
              <Dialog.Panel className='w-full max-w-2xl transform overflow-hidden rounded-2xl bg-card-light dark:bg-card-dark border border-borderline-light dark:border-borderline-dark p-6 text-left align-middle shadow-xl transition-all'>
                <Dialog.Title
                  as='h3'
                  className='text-xl font-bold leading-6 text-content-main-light dark:text-content-main-dark mb-4 border-b border-borderline-light dark:border-borderline-dark pb-3'
                >
                  {isEditMode
                    ? t('editTaskTitle') || 'Edit Task'
                    : t('addTaskTitle') || 'Create New Task'}
                </Dialog.Title>
                ...
                <div className='mt-4 space-y-4'>
                  <div>
                    ...
                    <input
                      ...
                      placeholder={
                        t('placeholderNewTask') || 'Enter task title...'
                      }
                  ...
                  <div>
                    ...
                    <div
                      className={`border border-borderline-light dark:border-borderline-dark rounded-xl p-3 bg-background-light dark:bg-background-dark transition-colors ${isSubmitting ? 'opacity-50 pointer-events-none' : 'focus-within:border-accent'}`}
                    >
                      <div className='border-b border-borderline-light dark:border-borderline-dark pb-2 mb-2 flex gap-2 text-content-sub-light dark:text-content-sub-dark'>
                        <button
                          ...
                          className={`p-1 rounded ${editor?.isActive('bold') ? 'bg-surface-light dark:bg-surface-dark text-accent' : 'hover:bg-surface-light dark:hover:bg-surface-dark'}`}
                        >
                        ...
                        <button
                          ...
                          className={`p-1 rounded ${editor?.isActive('italic') ? 'bg-surface-light dark:bg-surface-dark text-accent' : 'hover:bg-surface-light dark:hover:bg-surface-dark'}`}
                        >
                        ...
                        <button
                          ...
                          className={`p-1 rounded ${editor?.isActive('strike') ? 'bg-surface-light dark:bg-surface-dark text-accent' : 'hover:bg-surface-light dark:hover:bg-surface-dark'}`}
                        >
                        ...
                        <button
                          ...
                          className={`p-1 rounded ${editor?.isActive('bulletList') ? 'bg-surface-light dark:bg-surface-dark text-accent' : 'hover:bg-surface-light dark:hover:bg-surface-dark'}`}
                        >
                        ...
                        <button
                          ...
                          className={`p-1 rounded ${editor?.isActive('orderedList') ? 'bg-surface-light dark:bg-surface-dark text-accent' : 'hover:bg-surface-light dark:hover:bg-surface-dark'}`}
                        >
